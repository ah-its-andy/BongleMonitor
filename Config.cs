...
//SIM IMSI             : 454120640655691
        ...
        {
            var shortDev = System.IO.Path.GetFileNameWithoutExtension(dev);
            return $"# /tmp/ppp/peers/{shortDev}/ppp {Environment.NewLine}" +
            ...
            $"connect '/usr/sbin/chat -s -v -f /tmp/ppp/peers/{shortDev}/ppp-chat-connect'{Environment.NewLine}" +
            $"# 断开连接脚本{Environment.NewLine}" +
            $"disconnect '/usr/sbin/chat -s -v -f /tmp/ppp/peers/{shortDev}/ppp-chat-disconnect'{Environment.NewLine}" +
